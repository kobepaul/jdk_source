                           ...
                           implements java.util.Deque<E>, Cloneable, Serializable
{
    ...
    private void grow(int needed) {
        ...
        final Object[] es = elements = java.util.Arrays.copyOf(elements, newCapacity);
    ...
    private <T> T[] toArray(Class<T[]> klazz) {
        ...
        if ((end = tail + ((head <= tail) ? 0 : es.length)) >= 0) {
            ...
            a = java.util.Arrays.copyOfRange(es, head, end, klazz);
        } else {
            // integer overflow!
            a = java.util.Arrays.copyOfRange(es, 0, end - head, klazz);
