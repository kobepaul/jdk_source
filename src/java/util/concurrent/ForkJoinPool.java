...
public class ForkJoinPool extends AbstractExecutorService {
            ...
            while (s >= 0) {
                ...
                if ((s = task.status) < 0)
                    ...
                    if (deadline == 0L)
                        ...
                        break;                         // timeout
                    else if ((ms = java.util.concurrent.TimeUnit.NANOSECONDS.toMillis(ns)) <= 0L)
    ...
    public ForkJoinPool() {
        this(Math.min(MAX_CAP, Runtime.getRuntime().availableProcessors()),
             ...
             0, MAX_CAP, 1, null, DEFAULT_KEEPALIVE, java.util.concurrent.TimeUnit.MILLISECONDS);
    }
     ...
     * timeout elapsed.
     */
    public boolean awaitQuiescence(long timeout, java.util.concurrent.TimeUnit unit) {
        long nanos = unit.toNanos(timeout);
