...
class CharPredicates {
    ...
    static CharPredicate forProperty(String name) {
        ...
        switch (name) {
            ...
            case "Alnum": return ctype(java.util.regex.ASCII.ALNUM);  // Alphanumeric characters
            case "Alpha": return ctype(java.util.regex.ASCII.ALPHA);  // Alphabetic characters
            case "Blank": return ctype(java.util.regex.ASCII.BLANK);  // Space and tab characters
            case "Cntrl": return ctype(java.util.regex.ASCII.CNTRL);  // Control characters
            case "Digit": return range('0', '9');     // Numeric characters
            case "Graph": return ctype(java.util.regex.ASCII.GRAPH);  // printable and visible
            case "Lower": return range('a', 'z');     // Lower-case alphabetic
            case "Print": return range(0x20, 0x7E);   // Printable characters
            case "Punct": return ctype(java.util.regex.ASCII.PUNCT);  // Punctuation characters
            case "Space": return ctype(java.util.regex.ASCII.SPACE);  // Space characters
            case "Upper": return range('A', 'Z');     // Upper-case alphabetic
            case "XDigit": return ctype(java.util.regex.ASCII.XDIGIT); // hexadecimal digits
    ...
    private static CharPredicate ctype(final int ctype) {
        return (BmpCharPredicate)ch -> ch < 128 && java.util.regex.ASCII.isType(ch, ctype);
    }
    ...
    static final BmpCharPredicate ASCII_DIGIT() {
        return ch -> ch < 128 && java.util.regex.ASCII.isDigit(ch);
    }
    static final BmpCharPredicate ASCII_WORD() {
        return ch -> ch < 128 && java.util.regex.ASCII.isWord(ch);
    }
    static final BmpCharPredicate ASCII_SPACE() {
        return ch -> ch < 128 && java.util.regex.ASCII.isSpace(ch);
    }
