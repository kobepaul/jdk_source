...
final class WhileOps {
     ...
     */
    static <T> java.util.stream.Stream<T> makeTakeWhileRef(AbstractPipeline<?, T, ?> upstream,
        ...
        return new ReferencePipeline.StatefulOp<T, T>(upstream, StreamShape.REFERENCE, TAKE_FLAGS) {
            ...
            <P_IN> Spliterator<T> opEvaluateParallelLazy(PipelineHelper<T> helper,
                ...
                if (StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    return opEvaluateParallel(helper, spliterator, java.util.stream.Nodes.castingArray())
                            .spliterator();
                }
                else {
    ...
    static IntStream makeTakeWhileInt(AbstractPipeline<?, Integer, ?> upstream,
        ...
        return new IntPipeline.StatefulOp<Integer>(upstream, StreamShape.INT_VALUE, TAKE_FLAGS) {
            ...
            @Override
            <P_IN> java.util.stream.Node<Integer> opEvaluateParallel(PipelineHelper<Integer> helper,
                                                                     Spliterator<P_IN> spliterator,
    ...
    static LongStream makeTakeWhileLong(AbstractPipeline<?, Long, ?> upstream,
        ...
        return new LongPipeline.StatefulOp<Long>(upstream, StreamShape.LONG_VALUE, TAKE_FLAGS) {
            ...
            @Override
            <P_IN> java.util.stream.Node<Long> opEvaluateParallel(PipelineHelper<Long> helper,
                                                                  Spliterator<P_IN> spliterator,
    ...
    static DoubleStream makeTakeWhileDouble(AbstractPipeline<?, Double, ?> upstream,
        ...
        return new DoublePipeline.StatefulOp<Double>(upstream, StreamShape.DOUBLE_VALUE, TAKE_FLAGS) {
            ...
            @Override
            <P_IN> java.util.stream.Node<Double> opEvaluateParallel(PipelineHelper<Double> helper,
                                                                    Spliterator<P_IN> spliterator,
    ...
    interface DropWhileOp<T> {
         ...
         */
        DropWhileSink<T> opWrapSink(java.util.stream.Sink<T> sink, boolean retainAndCountDroppedElements);
    }
     ...
     * @param <T> the type of both input and output elements
     */
    interface DropWhileSink<T> extends java.util.stream.Sink<T> {
        /**
    ...
    static <T> Stream<T> makeDropWhileRef(AbstractPipeline<?, T, ?> upstream,
        ...
        class Op extends ReferencePipeline.StatefulOp<T, T> implements DropWhileOp<T> {
            ...
            public DropWhileSink<T> opWrapSink(java.util.stream.Sink<T> sink, boolean retainAndCountDroppedElements) {
                class OpSink extends java.util.stream.Sink.ChainedReference<T, T> implements DropWhileSink<T> {
                    long dropCount;
                    boolean take;
    ...
    static IntStream makeDropWhileInt(AbstractPipeline<?, Integer, ?> upstream,
        ...
        class Op extends IntPipeline.StatefulOp<Integer> implements DropWhileOp<Integer> {
            ...
            public DropWhileSink<Integer> opWrapSink(java.util.stream.Sink<Integer> sink, boolean retainAndCountDroppedElements) {
                class OpSink extends java.util.stream.Sink.ChainedInt<Integer> implements DropWhileSink<Integer> {
                    long dropCount;
                    boolean take;
    ...
    static LongStream makeDropWhileLong(AbstractPipeline<?, Long, ?> upstream,
        ...
        class Op extends LongPipeline.StatefulOp<Long> implements DropWhileOp<Long> {
            ...
            public DropWhileSink<Long> opWrapSink(java.util.stream.Sink<Long> sink, boolean retainAndCountDroppedElements) {
                class OpSink extends java.util.stream.Sink.ChainedLong<Long> implements DropWhileSink<Long> {
                    long dropCount;
                    boolean take;
    ...
    static DoubleStream makeDropWhileDouble(AbstractPipeline<?, Double, ?> upstream,
        ...
        class Op extends DoublePipeline.StatefulOp<Double> implements DropWhileOp<Double> {
            ...
            public DropWhileSink<Double> opWrapSink(java.util.stream.Sink<Double> sink, boolean retainAndCountDroppedElements) {
                class OpSink extends java.util.stream.Sink.ChainedDouble<Double> implements DropWhileSink<Double> {
                    long dropCount;
                    boolean take;
    ...
    abstract static class UnorderedWhileSpliterator<T, T_SPLITR extends Spliterator<T>> implements Spliterator<T> {
        ...
        abstract static class OfRef<T> extends UnorderedWhileSpliterator<T, Spliterator<T>> implements Consumer<T> {
            ...
            OfRef(Spliterator<T> s, java.util.stream.WhileOps.UnorderedWhileSpliterator.OfRef<T> parent) {
                super(s, parent);
                this.p = parent.p;
            }
            ...
            static final class Taking<T> extends java.util.stream.WhileOps.UnorderedWhileSpliterator.OfRef<T> {
                Taking(Spliterator<T> s, boolean noSplitting, Predicate<? super T> p) {
                    super(s, noSplitting, p);
                }

                Taking(Spliterator<T> s, java.util.stream.WhileOps.UnorderedWhileSpliterator.OfRef.Taking<T> parent) {
                    super(s, parent);
                }
                ...
                Spliterator<T> makeSpliterator(Spliterator<T> s) {
                    return new java.util.stream.WhileOps.UnorderedWhileSpliterator.OfRef.Taking<>(s, this);
                }
            }

            static final class Dropping<T> extends java.util.stream.WhileOps.UnorderedWhileSpliterator.OfRef<T> {
                Dropping(Spliterator<T> s, boolean noSplitting, Predicate<? super T> p) {
                    super(s, noSplitting, p);
                }

                Dropping(Spliterator<T> s, java.util.stream.WhileOps.UnorderedWhileSpliterator.OfRef.Dropping<T> parent) {
                    super(s, parent);
                }
                ...
                @Override
                Spliterator<T> makeSpliterator(Spliterator<T> s) {
                    return new java.util.stream.WhileOps.UnorderedWhileSpliterator.OfRef.Dropping<>(s, this);
                }
            }
        }
        ...
        abstract static class OfInt extends UnorderedWhileSpliterator<Integer, Spliterator.OfInt> implements IntConsumer, Spliterator.OfInt {
            ...
            static final class Taking extends UnorderedWhileSpliterator.OfInt {
                ...
                Spliterator.OfInt makeSpliterator(Spliterator.OfInt s) {
                    return new java.util.stream.WhileOps.UnorderedWhileSpliterator.OfInt.Taking(s, this);
                }
            }
            ...
            static final class Dropping extends UnorderedWhileSpliterator.OfInt {
                ...
                @Override
                Spliterator.OfInt makeSpliterator(Spliterator.OfInt s) {
                    return new java.util.stream.WhileOps.UnorderedWhileSpliterator.OfInt.Dropping(s, this);
                }
            }
        }
        ...
        abstract static class OfLong extends UnorderedWhileSpliterator<Long, Spliterator.OfLong> implements LongConsumer, Spliterator.OfLong {
            ...
            static final class Taking extends UnorderedWhileSpliterator.OfLong {
                ...
                Spliterator.OfLong makeSpliterator(Spliterator.OfLong s) {
                    return new java.util.stream.WhileOps.UnorderedWhileSpliterator.OfLong.Taking(s, this);
                }
            }
            ...
            static final class Dropping extends UnorderedWhileSpliterator.OfLong {
                ...
                @Override
                Spliterator.OfLong makeSpliterator(Spliterator.OfLong s) {
                    return new java.util.stream.WhileOps.UnorderedWhileSpliterator.OfLong.Dropping(s, this);
                }
            }
        }
        ...
        abstract static class OfDouble extends UnorderedWhileSpliterator<Double, Spliterator.OfDouble> implements DoubleConsumer, Spliterator.OfDouble {
            ...
            static final class Taking extends UnorderedWhileSpliterator.OfDouble {
                ...
                Spliterator.OfDouble makeSpliterator(Spliterator.OfDouble s) {
                    return new java.util.stream.WhileOps.UnorderedWhileSpliterator.OfDouble.Taking(s, this);
                }
            }
            ...
            static final class Dropping extends UnorderedWhileSpliterator.OfDouble {
                ...
                @Override
                Spliterator.OfDouble makeSpliterator(Spliterator.OfDouble s) {
                    return new java.util.stream.WhileOps.UnorderedWhileSpliterator.OfDouble.Dropping(s, this);
                }
            }
        }
    }
    ...
    private static final class TakeWhileTask<P_IN, P_OUT>
            extends AbstractShortCircuitTask<P_IN, P_OUT, java.util.stream.Node<P_OUT>, TakeWhileTask<P_IN, P_OUT>> {
        ...
        protected final java.util.stream.Node<P_OUT> doLeaf() {
            java.util.stream.Node.Builder<P_OUT> builder = helper.makeNodeBuilder(-1, generator);
            java.util.stream.Sink<P_OUT> s = op.opWrapSink(helper.getStreamAndOpFlags(), builder);
            ...
            java.util.stream.Node<P_OUT> node = builder.build();
        ...
        java.util.stream.Node<P_OUT> merge() {
            if (leftChild.thisNodeSize == 0) {
                ...
                return java.util.stream.Nodes.conc(op.getOutputShape(),
    ...
    private static final class DropWhileTask<P_IN, P_OUT>
            extends AbstractTask<P_IN, P_OUT, java.util.stream.Node<P_OUT>, DropWhileTask<P_IN, P_OUT>> {
        private final AbstractPipeline<P_OUT, P_OUT, ?> op;
        private final IntFunction<P_OUT[]> generator;
        private final boolean isOrdered;
        private long thisNodeSize;
        ...
        protected final java.util.stream.Node<P_OUT> doLeaf() {
            ...
            long sizeIfKnown = isChild && isOrdered && StreamOpFlag.SIZED.isPreserved(op.sourceOrOpFlags)
                               ...
                               : -1;
            java.util.stream.Node.Builder<P_OUT> builder = helper.makeNodeBuilder(sizeIfKnown, generator);
            @SuppressWarnings("unchecked")
        ...
        public final void onCompletion(CountedCompleter<?> caller) {
            if (!isLeaf()) {
                ...
                thisNodeSize = leftChild.thisNodeSize + rightChild.thisNodeSize;
                java.util.stream.Node<P_OUT> result = merge();
        ...
        private java.util.stream.Node<P_OUT> merge() {
            if (leftChild.thisNodeSize == 0) {
                // If the left node size is 0 then
                // use the right node result
                return rightChild.getLocalResult();
