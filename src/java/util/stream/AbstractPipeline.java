 ...
 * @since 1.8
 */
abstract class AbstractPipeline<E_IN, E_OUT, S extends java.util.stream.BaseStream<E_OUT, S>>
        extends java.util.stream.PipelineHelper<E_OUT> implements BaseStream<E_OUT, S> {
    ...
    AbstractPipeline(Supplier<? extends Spliterator<?>> source,
        ...
        this.sourceOrOpFlags = sourceFlags & java.util.stream.StreamOpFlag.STREAM_MASK;
        // The following is an optimization of:
        // StreamOpFlag.combineOpFlags(sourceOrOpFlags, StreamOpFlag.INITIAL_OPS_VALUE);
        this.combinedFlags = (~(sourceOrOpFlags << 1)) & java.util.stream.StreamOpFlag.INITIAL_OPS_VALUE;
        this.depth = 0;
    ...
    AbstractPipeline(AbstractPipeline<?, E_IN, ?> previousStage, int opFlags) {
        ...
        this.sourceOrOpFlags = opFlags & java.util.stream.StreamOpFlag.OP_MASK;
        this.combinedFlags = java.util.stream.StreamOpFlag.combineOpFlags(opFlags, previousStage.combinedFlags);
     ...
     * @return the result
     */
    final <R> R evaluate(java.util.stream.TerminalOp<E_OUT, R> terminalOp) {
    ...
    final java.util.stream.Node<E_OUT> evaluateToArrayNode(IntFunction<E_OUT[]> generator) {
        if (linkedOrConsumed)
            throw new IllegalStateException(MSG_STREAM_LINKED);
        linkedOrConsumed = true;
    ...
    final int getStreamFlags() {
        return java.util.stream.StreamOpFlag.toStreamFlags(combinedFlags);
    }
        ...
        if (isParallel() && sourceStage.sourceAnyStateful) {
            ...
            for (@SuppressWarnings("rawtypes") AbstractPipeline u = sourceStage, p = sourceStage.nextStage, e = this;
                ...
                if (p.opIsStateful()) {
                    ...
                    if (java.util.stream.StreamOpFlag.SHORT_CIRCUIT.isKnown(thisOpFlags)) {
                        // Clear the short circuit flag for next pipeline stage
                        // This stage encapsulates short-circuiting, the next
                        // stage may not have any short-circuit operations, and
                        // if so spliterator.forEachRemaining should be used
                        // for traversal
                        thisOpFlags = thisOpFlags & ~java.util.stream.StreamOpFlag.IS_SHORT_CIRCUIT;
                    }
                    ...
                    thisOpFlags = spliterator.hasCharacteristics(Spliterator.SIZED)
                            ? (thisOpFlags & ~java.util.stream.StreamOpFlag.NOT_SIZED) | java.util.stream.StreamOpFlag.IS_SIZED
                            : (thisOpFlags & ~java.util.stream.StreamOpFlag.IS_SIZED) | java.util.stream.StreamOpFlag.NOT_SIZED;
                }
                p.depth = depth++;
                p.combinedFlags = java.util.stream.StreamOpFlag.combineOpFlags(thisOpFlags, u.combinedFlags);
            }
        }

        if (terminalFlags != 0)  {
            // Apply flags from the terminal operation to last pipeline stage
            combinedFlags = java.util.stream.StreamOpFlag.combineOpFlags(terminalFlags, combinedFlags);
        }
    ...
    final <P_IN> long exactOutputSizeIfKnown(Spliterator<P_IN> spliterator) {
        return java.util.stream.StreamOpFlag.SIZED.isKnown(getStreamAndOpFlags()) ? spliterator.getExactSizeIfKnown() : -1;
    }

    @Override
    final <P_IN, S extends java.util.stream.Sink<E_OUT>> S wrapAndCopyInto(S sink, Spliterator<P_IN> spliterator) {
    ...
    final <P_IN> void copyInto(java.util.stream.Sink<P_IN> wrappedSink, Spliterator<P_IN> spliterator) {
        ...
        if (!java.util.stream.StreamOpFlag.SHORT_CIRCUIT.isKnown(getStreamAndOpFlags())) {
    ...
    final boolean isOrdered() {
        return java.util.stream.StreamOpFlag.ORDERED.isKnown(combinedFlags);
    }

    @Override
    @SuppressWarnings("unchecked")
    final <P_IN> java.util.stream.Sink<P_IN> wrapSink(java.util.stream.Sink<E_OUT> sink) {
        Objects.requireNonNull(sink);

        for ( @SuppressWarnings("rawtypes") AbstractPipeline p=AbstractPipeline.this; p.depth > 0; p=p.previousStage) {
            sink = p.opWrapSink(p.previousStage.combinedFlags, sink);
        }
        return (java.util.stream.Sink<P_IN>) sink;
    }
    ...
    @Override
    @SuppressWarnings("unchecked")
    final <P_IN> java.util.stream.Node<E_OUT> evaluate(Spliterator<P_IN> spliterator,
                                                       boolean flatten,
        ...
        if (isParallel()) {
            ...
            java.util.stream.Node.Builder<E_OUT> nb = makeNodeBuilder(
     ...
     */
    abstract <P_IN> java.util.stream.Node<E_OUT> evaluateToNode(java.util.stream.PipelineHelper<E_OUT> helper,
     ...
     */
    abstract <P_IN> Spliterator<E_OUT> wrap(java.util.stream.PipelineHelper<E_OUT> ph,
     ...
     */
    abstract boolean forEachWithCancel(Spliterator<E_OUT> spliterator, java.util.stream.Sink<E_OUT> sink);
     ...
     */
    @Override
    abstract java.util.stream.Node.Builder<E_OUT> makeNodeBuilder(long exactSizeIfKnown,
                                                                  IntFunction<E_OUT[]> generator);
     ...
     * then the method
     * {@link #opEvaluateParallel(java.util.stream.PipelineHelper, Spliterator, IntFunction)}
     * must be overridden.
     *
     ...
     *         {@code Sink}.
     */
    abstract java.util.stream.Sink<E_IN> opWrapSink(int flags, java.util.stream.Sink<E_OUT> sink);
     ...
     */
    <P_IN> java.util.stream.Node<E_OUT> opEvaluateParallel(java.util.stream.PipelineHelper<E_OUT> helper,
                                                           Spliterator<P_IN> spliterator,
                                                           IntFunction<E_OUT[]> generator) {
     ...
     */
    @SuppressWarnings("unchecked")
    <P_IN> Spliterator<E_OUT> opEvaluateParallelLazy(java.util.stream.PipelineHelper<E_OUT> helper,
                                                     Spliterator<P_IN> spliterator) {
