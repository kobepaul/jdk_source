...
final class RBTableBuilder {
     ...
     * @exception java.text.ParseException If the rules format is incorrect.
     */

    public void build(String pattern, int decmp) throws java.text.ParseException
    {
        ...
        String groupChars;
        if (pattern.length() == 0)
            throw new java.text.ParseException("Build rules empty.", 0);
        ...
        {
            java.text.PatternEntry entry = mPattern.getItemAt(i);
            if (entry != null) {
                                          ...
                                          boolean fwd)
    {
        ...
        Vector<java.text.EntryPair> entryTable = getContractValuesImpl(entry - RBCollationTables.CONTRACTCHARINDEX);
        ...
        if (entryTable == null) {
            ...
            entryTable.addElement(new java.text.EntryPair(groupChars.substring(0,Character.charCount(ch)), entry));
        ...
        if (index != RBCollationTables.UNMAPPED) {
            java.text.EntryPair pair = entryTable.elementAt(index);
            pair.value = anOrder;
        } else {
            java.text.EntryPair pair = entryTable.lastElement();
            ...
            if (groupChars.length() > pair.entryName.length()) {
                entryTable.addElement(new java.text.EntryPair(groupChars, anOrder, fwd));
            } else {
                entryTable.insertElementAt(new java.text.EntryPair(groupChars, anOrder,
    ...
    private int getContractOrder(String groupChars)
    {
        ...
        if (contractTable != null) {
            ...
            int ch = Character.isHighSurrogate(ch0)?
              Character.toCodePoint(ch0, groupChars.charAt(1)):ch0;
              */
            Vector<java.text.EntryPair> entryTable = getContractValues(ch);
            if (entryTable != null) {
    ...
    private final int getCharOrder(int ch) {
        ...
        if (order >= RBCollationTables.CONTRACTCHARINDEX) {
            Vector<java.text.EntryPair> groupList = getContractValuesImpl(order - RBCollationTables.CONTRACTCHARINDEX);
            java.text.EntryPair pair = groupList.firstElement();
            order = pair.value;
        }
        return order;
    }
     ...
     */
    private Vector<java.text.EntryPair> getContractValues(int ch)
    {
    ...
    private final void addExpandOrder(String contractChars,
                                String expandChars,
                                int anOrder) throws java.text.ParseException
    {
