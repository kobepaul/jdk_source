...
public final class CollationElementIterator
{
     ...
     */
    CollationElementIterator(String sourceText, java.text.RuleBasedCollator owner) {
        this.owner = owner;
     ...
     * @param owner the collation object.
     */
    CollationElementIterator(java.text.CharacterIterator sourceText, java.text.RuleBasedCollator owner) {
        this.owner = owner;
        ordering = owner.getTables();
        NormalizerBase.Mode mode =
            CollatorUtilities.toNormalizerMode(owner.getDecomposition());
    ...
    public int next()
    {
        ...
        if (value == java.text.RuleBasedCollator.UNMAPPED) {
            swapOrder = ch;
            return UNMAPPEDCHARVALUE;
        }
        else if (value >= java.text.RuleBasedCollator.CONTRACTCHARINDEX) {
            value = nextContractChar(ch);
        }
        if (value >= java.text.RuleBasedCollator.EXPANDCHARINDEX) {
    ...
    {
        order &= java.text.RBCollationTables.PRIMARYORDERMASK;
        return (order >>> java.text.RBCollationTables.PRIMARYORDERSHIFT);
    }
    /**
    ...
    {
        order = order & java.text.RBCollationTables.SECONDARYORDERMASK;
        return ((short)(order >> java.text.RBCollationTables.SECONDARYORDERSHIFT));
    }
    /**
    ...
    {
        return ((short)(order &= java.text.RBCollationTables.TERTIARYORDERMASK));
    }
    ...
    final int strengthOrder(int order)
    {
        ...
        if (s == java.text.Collator.PRIMARY)
        {
            order &= java.text.RBCollationTables.PRIMARYDIFFERENCEONLY;
        } else if (s == Collator.SECONDARY)
        {
            order &= java.text.RBCollationTables.SECONDARYDIFFERENCEONLY;
        }
        return order;
    }
    ...
    private int[] makeReorderedBuffer(int colFirst,
        ...
        if (firstValue >= java.text.RuleBasedCollator.CONTRACTCHARINDEX) {
            firstValue = forward? nextContractChar(colFirst) : prevContractChar(colFirst);
        }

        int[] firstExpansion = null;
        if (firstValue >= java.text.RuleBasedCollator.EXPANDCHARINDEX) {
    ...
    private int nextContractChar(int ch)
    {
        ...
        Vector<java.text.EntryPair> list = ordering.getContractValues(ch);
        java.text.EntryPair pair = list.firstElement();
