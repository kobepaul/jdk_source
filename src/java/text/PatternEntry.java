...
class PatternEntry {
                     ...
                     PatternEntry lastEntry)
    {
        if (showWhiteSpace && toAddTo.length() > 0)
            if (strength == java.text.Collator.PRIMARY || lastEntry != null)
                toAddTo.append('\n');
            else
                toAddTo.append(' ');
        if (lastEntry != null) {
        ...
        switch (strength) {
        case java.text.Collator.IDENTICAL: toAddTo.append('='); break;
        case java.text.Collator.TERTIARY:  toAddTo.append(','); break;
        case java.text.Collator.SECONDARY: toAddTo.append(';'); break;
        case java.text.Collator.PRIMARY:   toAddTo.append('<'); break;
        ...
        mainLoop:
            while (i < pattern.length()) {
                ...
                case '=': if (newStrength != UNSET) break mainLoop;
                    newStrength = java.text.Collator.IDENTICAL; break;
                case ',': if (newStrength != UNSET) break mainLoop;
                    newStrength = java.text.Collator.TERTIARY; break;
                case ';': if (newStrength != UNSET) break mainLoop;
                    newStrength = java.text.Collator.SECONDARY; break;
                case '<': if (newStrength != UNSET) break mainLoop;
