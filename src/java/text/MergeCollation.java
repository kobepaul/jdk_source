...
final class MergeCollation {
    ...
    public String getPattern(boolean withWhiteSpace) {
        ...
        java.text.PatternEntry tmp = null;
        ArrayList<java.text.PatternEntry> extList = null;
        int i;
        for (i = 0; i < patterns.size(); ++i) {
            java.text.PatternEntry entry = patterns.get(i);
            if (entry.extension.length() != 0) {
                ...
                if (extList != null) {
                    java.text.PatternEntry last = findLastWithNoExtension(i-1);
    ...
    private final java.text.PatternEntry findLastWithNoExtension(int i) {
        for (--i;i >= 0; --i) {
            java.text.PatternEntry entry = patterns.get(i);
            if (entry.extension.length() == 0) {
                return entry;
    ...
    public void addPattern(String pattern) throws ParseException
    {
        ...
        java.text.PatternEntry.Parser parser = new java.text.PatternEntry.Parser(pattern);

        java.text.PatternEntry entry = parser.next();
        while (entry != null) {
            fixEntry(entry);
            entry = parser.next();
        }
     ...
     */
    public java.text.PatternEntry getItemAt(int index) {
    ...
    // privates
    //============================================================
    ArrayList<java.text.PatternEntry> patterns = new ArrayList<>(); // a list of PatternEntries

    private transient java.text.PatternEntry saveEntry = null;
    private transient java.text.PatternEntry lastEntry = null;
    ...
    /*
      ...
      */
    private final void fixEntry(java.text.PatternEntry newEntry) throws ParseException
    {
        ...
        if (lastEntry != null && newEntry.chars.equals(lastEntry.chars)
            ...
            if (newEntry.strength != Collator.IDENTICAL
                && newEntry.strength != java.text.PatternEntry.RESET) {
        ...
        if (newEntry.strength != java.text.PatternEntry.RESET) {
            int oldIndex = -1;
    ...
    private final int findLastEntry(java.text.PatternEntry entry,
                                    StringBuffer excessChars) throws ParseException
    {
        if (entry == null)
            return 0;

        if (entry.strength != java.text.PatternEntry.RESET) {
            ...
            for (i = patterns.size() - 1; i >= 0; --i) {
                java.text.PatternEntry e = patterns.get(i);
