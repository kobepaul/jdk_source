...
public class RuleBasedCollator extends java.text.Collator {
     ...
     * @exception java.text.ParseException A format exception
     * will be thrown if the build process of the rules fails. For
     * example, build rule "a &lt; ? &lt; d" will cause the constructor to
     * throw the ParseException because the '?' is not quoted.
     */
    public RuleBasedCollator(String rules) throws java.text.ParseException {
        this(rules, java.text.Collator.CANONICAL_DECOMPOSITION);
    }
    ...
    RuleBasedCollator(String rules, int decomp) throws ParseException {
        ...
        tables = new java.text.RBCollationTables(rules, decomp);
    }
    ...
    public synchronized int compare(String source, String target)
    {
        ...
        int result = java.text.Collator.EQUAL;
        ...
        boolean initialCheckSecTer = getStrength() >= java.text.Collator.SECONDARY;
        boolean checkSecTer = initialCheckSecTer;
        boolean checkTertiary = getStrength() >= java.text.Collator.TERTIARY;
        ...
        while(true) {
            ...
            if ( pSOrder != pTOrder )
            {
                ...
                if (pSOrder == 0)  // primary order in source is ignorable
                {
                    ...
                    if (checkSecTer) {
                        result = java.text.Collator.GREATER;  // (strength is SECONDARY)
                        checkSecTer = false;
                    }
                    // Skip to the next source element, but don't fetch another target element.
                    gett = false;
                ...
                else if (pTOrder == 0)
                {
                    ...
                    if (checkSecTer) {
                        result = java.text.Collator.LESS;  // (strength is SECONDARY)
                        checkSecTer = false;
                    }
                    // Skip to the next source element, but don't fetch another target element.
                    gets = false;
                    ...
                    if (pSOrder < pTOrder) {
                        return java.text.Collator.LESS;  // (strength is PRIMARY)
                    } else {
                        return java.text.Collator.GREATER;  // (strength is PRIMARY)
                    }
                }
                ...
                if (checkSecTer) {
                    ...
                    if (secSOrder != secTOrder) {
                        ...
                        result = (secSOrder < secTOrder) ? java.text.Collator.LESS : java.text.Collator.GREATER;
                        ...
                        if (checkTertiary) {
                            ...
                            if (terSOrder != terTOrder) {
                                ...
                                result = (terSOrder < terTOrder) ? java.text.Collator.LESS : java.text.Collator.GREATER;
    ...
    public synchronized CollationKey getCollationKey(String source)
    {
        ...
        int order = 0;
        boolean compareSec = (getStrength() >= java.text.Collator.SECONDARY);
            ...
            if (!CollationElementIterator.isIgnorable(order))
            {
                ...
                if (compareSec) {
                    ...
                    if (tables.isFrenchSec() && preSecIgnore < secResult.length()) {
                        ...
                        //
                        java.text.RBCollationTables.reverse(secResult, preSecIgnore, secResult.length());
                    }
        ...
        if (tables.isFrenchSec())
        {
            ...
            java.text.RBCollationTables.reverse(secResult, 0, secResult.length());
        }
    ...
    private java.text.RBCollationTables tables = null;
