...
final class StackStreamFactory {
    ...
    private final static Set<java.lang.Class<?>> stackWalkImplClasses = init();
    ...
    static <T> StackFrameTraverser<T>
        makeStackTraverser(java.lang.StackWalker walker, Function<? super Stream<StackFrame>, ? extends T> function)
    {
     ...
     */
    static CallerClassFinder makeCallerFinder(java.lang.StackWalker walker) {
    ...
    static abstract class AbstractStackWalker<R, T> {
        ...
        protected AbstractStackWalker(java.lang.StackWalker walker, int mode) {
            this(walker, mode, java.lang.Integer.MAX_VALUE);
        }
        protected AbstractStackWalker(java.lang.StackWalker walker, int mode, int maxDepth) {
        ...
        private int toStackWalkMode(java.lang.StackWalker walker, int mode) {
            int newMode = mode;
        ...
        protected int getNextBatchSize() {
            ...
            if (isDebug) {
                java.lang.System.err.println("last batch size = " + lastBatchSize +
        ...
        final void checkState(WalkerState state) {
            if (thread != java.lang.Thread.currentThread()) {
                throw new IllegalStateException("Invalid thread walking this stack stream: " +
                        java.lang.Thread.currentThread().getName() + " " + thread.getName());
            }
            switch (state) {
                case NEW:
                    if (anchor != 0) {
        ...
        final java.lang.Class<?> peekFrame() {
            while (frameBuffer.isActive() && depth < maxDepth) {
                if (frameBuffer.isEmpty()) {
                    ...
                    java.lang.Class<?> c = frameBuffer.get();
                    if (skipReflectionFrames() && isReflectionFrame(c)) {
                        if (isDebug)
                            java.lang.System.err.println("  skip: frame " + frameBuffer.getIndex() + " " + c);
         ...
         * 3. create specialized StackFrame objects
         */
        private java.lang.Object doStackWalk(long anchor, int skipFrames, int batchSize,
            ...
            if (isDebug) {
                java.lang.System.err.format("doStackWalk: skip %d start %d end %d%n",
        ...
        private int getNextBatch() {
            int nextBatchSize = java.lang.Math.min(maxDepth - depth, getNextBatchSize());
            if (!frameBuffer.isActive() || nextBatchSize <= 0) {
                if (isDebug) {
                    java.lang.System.out.format("  more stack walk done%n");
                }
        ...
        final java.lang.Class<?> nextFrame() {
            ...
            java.lang.Class<?> c = frameBuffer.next();
            depth++;
            return c;
        }
        ...
        private int fetchStackFrames(int batchSize) {
            ...
            int endIndex = fetchStackFrames(mode, anchor, batchSize,
                                            ...
                                            frameBuffer.frames());
            if (isDebug) {
                java.lang.System.out.format("  more stack walk requesting %d got %d to %d frames%n",
         ...
         *                    or a {@link java.lang.StackFrameInfo} (or derivative) array otherwise.
     ...
     */
    static class StackFrameTraverser<T> extends AbstractStackWalker<T, java.lang.StackFrameInfo>
            implements Spliterator<StackFrame>
    {
        ...
        final class StackFrameBuffer extends FrameBuffer<java.lang.StackFrameInfo> {
            private java.lang.StackFrameInfo[] stackFrames;
            StackFrameBuffer(int initialBatchSize) {
                super(initialBatchSize);

                this.stackFrames = new java.lang.StackFrameInfo[initialBatchSize];
                for (int i = START_POS; i < initialBatchSize; i++) {
                    stackFrames[i] = new java.lang.StackFrameInfo(walker);
                }
            }

            @java.lang.Override
            java.lang.StackFrameInfo[] frames() {
                return stackFrames;
            }
            ...
            void resize(int startIndex, int elements) {
                ...
                if (stackFrames.length < size) {
                    java.lang.StackFrameInfo[] newFrames = new java.lang.StackFrameInfo[size];
                    // copy initial magic...
                    java.lang.System.arraycopy(stackFrames, 0, newFrames, 0, startIndex);
            ...
            @java.lang.Override
            java.lang.StackFrameInfo nextStackFrame() {
                if (isEmpty()) {
                    throw new NoSuchElementException("origin=" + origin + " fence=" + fence);
                }

                java.lang.StackFrameInfo frame = stackFrames[origin];
                origin++;
                return frame;
            }

            @java.lang.Override
            final java.lang.Class<?> at(int index) {
        ...
        StackFrameTraverser(java.lang.StackWalker walker,
        ...
        StackFrame nextStackFrame() {
            ...
            java.lang.StackFrameInfo frame = frameBuffer.nextStackFrame();
            depth++;
            return frame;
        }
        ...
        protected int batchSize(int lastBatchFrameCount) {
            if (lastBatchFrameCount == 0) {
                ...
                // and not too small
                int initialBatchSize = java.lang.Math.max(walker.estimateDepth(), SMALL_BATCH);
                return java.lang.Math.min(initialBatchSize, LARGE_BATCH_SIZE);
            } else {
                if (lastBatchFrameCount > BATCH_SIZE) {
                    return lastBatchFrameCount;
                } else {
                    return java.lang.Math.min(lastBatchFrameCount*2, BATCH_SIZE);
                }
            }
        }
        ...
        public boolean tryAdvance(Consumer<? super StackFrame> action) {
            ...
            if (hasNext()) {
                ...
                action.accept(frame);
                if (isDebug) {
                    java.lang.System.err.println("tryAdvance: " + index + " " + frame);
                }
                return true;
            }
            if (isDebug) {
                java.lang.System.err.println("tryAdvance: " + index + " NO element");
            }
            return false;
        }
    }
     ...
     */
    static final class CallerClassFinder extends AbstractStackWalker<java.lang.Integer, java.lang.Class<?>> {
        static {
            stackWalkImplClasses.add(CallerClassFinder.class);
        }

        private java.lang.Class<?> caller;

        CallerClassFinder(java.lang.StackWalker walker) {
            super(walker, FILL_CLASS_REFS_ONLY|GET_CALLER_CLASS);
        }

        final class ClassBuffer extends FrameBuffer<java.lang.Class<?>> {
            java.lang.Class<?>[] classes;      // caller class for fast path
            ClassBuffer(int batchSize) {
                super(batchSize);
                classes = new java.lang.Class<?>[batchSize];
            }

            @java.lang.Override
            java.lang.Class<?>[] frames() { return classes;}

            @java.lang.Override
            final java.lang.Class<?> at(int index) { return classes[index];}
            ...
            void resize(int startIndex, int elements) {
                ...
                if (classes.length < size) {
                    ...
                    java.lang.Class<?>[] prev = classes;
                    classes = new java.lang.Class<?>[size];
                    java.lang.System.arraycopy(prev, 0, classes, 0, startIndex);
                }
                currentBatchSize = size;
            }
        }

        java.lang.Class<?> findCaller() {
            walk();
            return caller;
        }

        @java.lang.Override
        protected java.lang.Integer consumeFrames() {
            checkState(OPEN);
            int n = 0;
            java.lang.Class<?>[] frames = new java.lang.Class<?>[2];
    ...
    static final class LiveStackInfoTraverser<T> extends StackFrameTraverser<T> {
        ...
        final class LiveStackFrameBuffer extends FrameBuffer<java.lang.LiveStackFrameInfo> {
            private java.lang.LiveStackFrameInfo[] stackFrames;
            LiveStackFrameBuffer(int initialBatchSize) {
                super(initialBatchSize);
                this.stackFrames = new java.lang.LiveStackFrameInfo[initialBatchSize];
                for (int i = START_POS; i < initialBatchSize; i++) {
                    stackFrames[i] = new java.lang.LiveStackFrameInfo(walker);
                }
            }

            @java.lang.Override
            java.lang.LiveStackFrameInfo[] frames() {
                return stackFrames;
            }
            ...
            void resize(int startIndex, int elements) {
                ...
                int size = startIndex + elements;
                if (stackFrames.length < size) {
                    java.lang.LiveStackFrameInfo[] newFrames = new java.lang.LiveStackFrameInfo[size];
                    // copy initial magic...
                    java.lang.System.arraycopy(stackFrames, 0, newFrames, 0, startIndex);
                    stackFrames = newFrames;
                }
            ...
            @java.lang.Override
            java.lang.LiveStackFrameInfo nextStackFrame() {
                if (isEmpty()) {
                    throw new NoSuchElementException("origin=" + origin + " fence=" + fence);
                }

                java.lang.LiveStackFrameInfo frame = stackFrames[origin];
                origin++;
                return frame;
            }
        ...
        LiveStackInfoTraverser(java.lang.StackWalker walker,
                               Function<? super Stream<StackFrame>, ? extends T> function) {
    ...
    static abstract class FrameBuffer<F> {
         ...
         * {@link java.lang.StackFrameInfo} (or derivative) array otherwise.
         *
         ...
         * @return the class at the given position in the current batch.
         */
        abstract java.lang.Class<?> at(int index);
         ...
         */
        F nextStackFrame() {
            throw new java.lang.InternalError("should not reach here");
        }
         ...
         */
        final java.lang.Class<?> next() {
            if (isEmpty()) {
                throw new NoSuchElementException("origin=" + origin + " fence=" + fence);
            }
            java.lang.Class<?> c = at(origin);
            origin++;
            if (isDebug) {
                int index = origin-1;
                java.lang.System.out.format("  next frame at %d: %s (origin %d fence %d)%n", index,
        ...
        final void setBatch(int depth, int startIndex, int endIndex) {
            ...
            if (depth == 0 && fence > 0) {
                ...
                for (int i = START_POS; i < fence; i++) {
                    java.lang.Class<?> c = at(i);
                    if (isDebug) java.lang.System.err.format("  frame %d: %s%n", i, c);
    ...
    private static Set<java.lang.Class<?>> init() {
        if (!checkStackWalkModes()) {
            throw new java.lang.InternalError("StackWalker mode values do not match with JVM");
        }

        Set<java.lang.Class<?>> classes = new HashSet<>();
        classes.add(java.lang.StackWalker.class);
        classes.add(StackStreamFactory.class);
        classes.add(AbstractStackWalker.class);
        return classes;
    }

    private static boolean filterStackWalkImpl(java.lang.Class<?> c) {
    ...
    // to filter them out
    private static boolean isMethodHandleFrame(java.lang.Class<?> c) {
        return c.getName().startsWith("java.lang.invoke.");
    }

    private static boolean isReflectionFrame(java.lang.Class<?> c) {
